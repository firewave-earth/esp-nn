                            ...
                            const quant_data_t *quant_data)
{
...

    {
         ...
         * filter loads use ee.vld.128.ip which silently aligns down to a
         * 16-byte boundary, so an unaligned scratch_buffer causes the kernel
         * to read 4..15 bytes of garbage at the start of every filter row.
         * align_buf_size in the size calc already reserves 16 bytes for this. */
